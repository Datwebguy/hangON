...
async function begin() {
  ...
  try {
    const sessionResponse = await fetch('/api/session', { credentials: 'same-origin' }); const sessionBody = await sessionResponse.json().catch(() => ({})); if (!sessionResponse.ok) throw new Error(sessionBody.error?.message || 'HangON could not create a session.'); csrfToken = sessionBody.data?.csrf || '';
