...
### Speech understanding for messy callers (LLM Gateway)
- Keyterms for trade language (`P-trap`, `GFCI breaker`, street names)
- The caller transcript goes to **AssemblyAI LLM Gateway** with a strict JSON schema, which resolves self-corrections (“Thursday… wait, Friday at 10:30”) and returns `null` for anything the caller never said
- Side-by-side “caller said” vs “booking summary” in the UI

### After the call (LLM Gateway)
- Technician brief, caller-mood labels, and parts checklist generated by **AssemblyAI LLM Gateway** from the real call transcript
- No local fallback: if the gateway is unreachable, the UI says so instead of showing canned output
...
| Variable | Purpose |
| --- | --- |
| `ASSEMBLYAI_API_KEY` | Live voice sessions and LLM Gateway extraction (server-side only) |
| `HANGON_LLM_MODEL` | Optional. LLM Gateway model, default `claude-haiku-4-5-20251001` |
